...
entry:
  %ptr = call double (double, double, i1)* (...) @__raptor_truncate_mem_func(double (double, double, i1)* @f, i64 64, i64 32)
...
entry:
  %ptr = call double (double, double, i1)* (...) @__raptor_truncate_op_func(double (double, double, i1)* @f, i64 64, i64 32)
  %res = call double %ptr(double %x, double %y, i1 %cond)
  ret double %res
}

; CHECK: define internal double @__raptor_done_truncate_mem_func_ieee_64toieee_32_f(double %x, double %y, i1 %cond) {
; CHECK-NEXT:   %res = select i1 %cond, double %x, double %y
; CHECK-NEXT:   ret double %res
; CHECK-NEXT: }

; CHECK: define internal double @__raptor_done_truncate_op_func_ieee_64toieee_32_f(double %x, double %y, i1 %cond) {
; CHECK-NEXT:   %res = select i1 %cond, double %x, double %y
; CHECK-NEXT:   ret double %res
; CHECK-NEXT: }
