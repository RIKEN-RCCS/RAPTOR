...
program trunc_f90
  ...
  interface
     function f__raptor_truncate_mem_func(tfunc, from_ieee, to_type, to_exponent, to_significand) result (fty) bind (c)
       use iso_c_binding
       implicit none
     ...
     function f__raptor_truncate_mem_value(x, from_ieee, to_type, to_exponent, to_significand) &
          result(fty) bind (c)
       use iso_c_binding
       implicit none
       ...
       integer(c_int), intent(in), value :: from_ieee, to_type, to_exponent, to_significand
       double precision :: fty
     end function f__raptor_truncate_mem_value

     function f__raptor_expand_mem_value(x, from_ieee, to_type, to_exponent, to_significand) &
          result(fty) bind(c)
       use :: iso_c_binding
       implicit none
  ...
  cfty = f__raptor_truncate_mem_func(cfty, 64, 1, 10, 4)
  call c_f_procpointer(cfty, ffty)

  ta = f__raptor_truncate_mem_value(a, 64, 1, 10, 4)
  tb = f__raptor_truncate_mem_value(b, 64, 1, 10, 4)
  tc = ffty(ta, tb)
  c = f__raptor_expand_mem_value(tc, 64, 1, 10, 4)
